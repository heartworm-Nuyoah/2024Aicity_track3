...
def inference_recognizer(model,
    ...
    for start_idx in trange(0,total_frames-16,step):
...
def parse_args():
    ...
    # parser.add_argument('--config', default='/disk2/js_data/AICITY/2023code/mmaction2-master/configs/Aicity/swin_base_patch244_window877_kinetics400_1k_right_minidata.py',help='test config file path')
    parser.add_argument('--config', default='/disk2/js_data/AICITY/2023code/mmaction2-master/configs/recognition/x3d/x3d_m_16x5x1_facebook_kinetics400_rgb_dash.py',help='test config file path')
    
    parser.add_argument('--checkpoint',default='/disk2/js_data/AICITY/2023code/mmaction2-master/runs/35_dash_x3d/epoch_13.pth', help='checkpoint file/url')
    # parser.add_argument('--video_data_root',default='/disk2/js_data/AICITY/datasets/2023/AI-City-Challenge-2023/A1_frame/Right_side_window_user_id_85870_NoAudio_5', help='data_root directory')
    parser.add_argument('--video_data_root',default='/disk2/js_data/AICITY/datasets/2023/AI-City-Challenge-2023/A1_frame', help='data_root directory')
    parser.add_argument('--label',default='/disk2/js_data/AICITY/2023code/mmaction2-master/label_map.txt', help='label file')
    parser.add_argument('--result_root',default='/disk2/js_data/AICITY/2023code/mmaction2-master/val_result',help='result_root')
    parser.add_argument('--view', default='dash', type=str, help='view of dash, rear, right')
    parser.add_argument('--val_id',default = '85870_86356_86952_96269_99882',help = 'val_id') #['85870','86356','86952','96269','99882']
    parser.add_argument(
        '--cfg-options',
        nargs='+',
        action=DictAction,
...
def main():
    ...
    if os.path.split(args.video_data_root)[1].lower().startswith(('dash','rear','right')):
        ...
        mkdir(args.result_root)
        
        result_root = os.path.join(args.result_root,
        ...
        # val_id = ['85870','86356','86952','96269','99882']
        str_val_id = args.val_id
        keywords = []
        for id_name in str_val_id.split('_'):
            keywords.append(id_name)
        # keywords = ['85870'] #TODO
        # keywords = args.val_id
        print(keywords)
        videos = list(filter(lambda s: any(str(keyword) in s for keyword in keywords), videos))
        # assign the desired device.
        device = torch.device(args.device)
        cfg = Config.fromfile(args.config)
        cfg.merge_from_dict(args.cfg_options)
        ...
        # result_root = os.path.join('/disk2/js_data/AICITY/2023code/mmaction2-master/submit_results_source/test')#,"{:04d}{:02d}{:02d}{:02d}{:02d}{:02d}".format(t.tm_year,t.tm_mon,t.tm_mday,t.tm_hour,t.tm_min,t.tm_sec))
        mkdir(result_root)
        result_root_path = os.path.join(result_root,args.view)
        mkdir(result_root_path)
        # mkdir(os.path.join(args.result_root,args.view))
        for video in tqdm(videos[args.start_end[0]:args.start_end[1]]):
            filen = result_root_path+'/'+video +'_'+ args.checkpoint.split('/')[-1].split('.')[0] + f'_results_{args.step}.txt'
            results = inference_recognizer(
