...
def show_result(label_path,rslt_file):
    ...
    for result in results:
        ...
        result = result[1:]
        flag=1
        flag=(flag+1)%2# print once each two lines.
        if flag==0:
            print('{:>02d}:{:>02.2f}--{:s}, \t{:.4f}'.format(int(clip_id / 8 // 60), (clip_id / 8 % 60),
                                                       label[result.argmax()], result.max()))
    return

def visual_result(label_path,rslt_file,input_folder,output_folder,Step):
    with open(label_path, 'r') as f:
        label = [line.strip() for line in f]
    results = np.loadtxt(rslt_file, delimiter=None)
    ...
    new_dir_name = dir_name + f'_step{Step}'
    new_path = os.path.join(output_folder, new_dir_name)
    mkdir(new_path)

    for result in results:
        clip_id = int(result[0])
        clip_id_str = '{:06d}'.format(clip_id)  #frame_id
        result = result[1:]
        a = label[result.argmax()] # a is the class, example:(14: yawning)

        img_path = os.path.join(input_folder, f'frame{clip_id_str}.jpg')
        img = cv2.imread(img_path)
        text1 = f'Frame {clip_id}'
        text2 = a
        ...
        color = (0, 0, 255)
        thickness = 2
        img = cv2.putText(img, text1, org1, font, fontScale, color, thickness)
        img = cv2.putText(img, text2, org2, font, fontScale, color, thickness)

        output_path = os.path.join(new_path, f'frame{clip_id_str}.jpg')
        cv2.imwrite(output_path, img)
    return

def parse_args():
    parser = argparse.ArgumentParser(description='MMAction2 demo')
    parser.add_argument('--config', default='/disk2/js_data/AICITY/2023code/mmaction2-master/runs/225_label/swin_base_patch244_window877_kinetics400_1k_dash_test2.py',help='test config file path')
    parser.add_argument('--checkpoint',default='/disk2/js_data/AICITY/2023code/mmaction2-master/runs/225_label/epoch_9.pth', help='checkpoint file/url')
    parser.add_argument('--video_data_root',default='/disk2/js_data/AICITY/datasets/2023/AI-City Challenge 2023/A2_frame', help='data_root directory')
...
def main():
    ...
    if os.path.split(args.video_data_root)[1].lower().startswith(('dash','rear','right')):
        ...
        model = init_recognizer(
            cfg, args.checkpoint, device=device, use_frames=args.use_frames)
        t=time.localtime(time.time())
        result_root = os.path.join('./submit_results_source/total/rear')#,"{:04d}{:02d}{:02d}{:02d}{:02d}{:02d}".format(t.tm_year,t.tm_mon,t.tm_mday,t.tm_hour,t.tm_min,t.tm_sec))
        mkdir(result_root)
