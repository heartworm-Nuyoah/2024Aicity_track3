...
class RawVideoDecode:
    ...
    def __init__(self, io_backend='disk', decoding_backend='cv2', pad_square= False, **kwargs):
    ...
    def modify_bbox(self, bbox, ori_shape):
        x1, y1, x2, y2 = bbox
        w = np.abs(x2 - x1)
        h = np.abs(y2 - y1)
        cy = (y1 + y2) // 2
        cx = (x1 + x2) // 2
        if cx < 0 or cx > ori_shape[1] or cy < 0 or cy > ori_shape[0]:
            cx = ori_shape[1] // 2
            cy = ori_shape[0] // 2
            w = ori_shape[1]
            h = ori_shape[0]
            return [cx - min(w, h) // 2 + 1, cy - min(w, h) // 2 + 1, cx + min(w, h) // 2 - 1, cy + min(w, h) // 2 - 1]

        if self.pad_square:
            if h < w:
                side = w * 1.2 # expand wider side

            else:
                side = h * 1.2 # expand wider side
            y1 = cy - (side // 2)
            y2 = cy + (side // 2)
            x1 = cx - (side // 2)
            x2 = cx + (side // 2)
        y1 = int(max(0, y1))
        y2 = int(min(y2, ori_shape[0]))
        x1 = int(max(0, x1))
        x2 = int(min(x2, ori_shape[1]))
        return [x1, y1, x2, y2]
    ...
    def __call__(self, results):
        ...
        v_c=cv2.VideoCapture(directory)
        for frame_idx in results['frame_inds']:
            frame_idx += offset
            frame_idx += results['fid']
            assert 'bbox_clip' in results.keys()
            bbox = list(map(int, results['bbox_clip']))
            fid=frame_idx
            while True:
                v_c.set(cv2.CAP_PROP_POS_FRAMES, fid)  #
                ret, img_ori = v_c.read()
                if ret:
                    break
                fid = fid -1
                assert fid!=-1, f"{directory} is None."
            img_ori = cv2.cvtColor(img_ori, cv2.COLOR_BGR2RGB)
            try:
                x1,y1,x2,y2=self.modify_bbox(bbox,img_ori.shape)
                img_crop = img_ori[y1:y2, x1:x2]
                cur_frame = img_crop
            except:
                assert len(imgs) > 0
                cur_frame = imgs[-1]
            if cur_frame is None or 0 in cur_frame.shape:
                print(f"{directory} shape is wrong on {frame_idx}th frame.")
            imgs.append(cur_frame)

        def showimg(x,  ind, txt = 'Flip:'):
            import matplotlib.pyplot as plt
            # import numpy as np
            clip_len = len(x)#.shape[2]
            img = x#(x.to('cpu').detach()[ind]).permute(1, 2, 3, 0).numpy()
            for i in range(clip_len):
                plt.subplot(2, 4, i + 1)
                plt.imshow(img[i].astype('uint8'))
            plt.subplot(2,4,1)
            plt.text(0, img.shape[1] * 2, txt)
            plt.show()

        v_c.release()
        # showimg(np.array(imgs),  ind=1,txt = "Flip: {}. Label:{}".format(results['flip'],np.argmax(results['label'])))
        results['imgs'] = imgs
...
class FadSampleFrames:
    ...
    def __init__(self,
        ...
        self.temporal_flip_r = temporal_flip_r
    ...
    def __call__(self, results):
        ...
        frame_inds = []
        interval = (total_frames / self.clip_len) * (np.arange(self.clip_len + 1))
        self.frame_interval = max(round(total_frames / self.clip_len), 1)
        if not self.test_mode:
            if total_frames > self.clip_len:
                for i in range(0, self.clip_len, 2):
                    frame_inds += list(
                        np.sort(np.random.choice(int(np.round(interval[i + 2] - interval[i])), 2, replace=False)) + int(
                            np.round(interval[i])))
            else:
                frame_inds = list(np.round(interval[:-1]).astype('int'))
        else:
            self.frame_interval = max(round(total_frames / self.clip_len), 1)
            if total_frames > self.clip_len*2:
                frame_inds = list(np.round(interval[:-1]).astype('int')+self.frame_interval//2)

            else:
                frame_inds = list(np.round(interval[:-1]).astype('int'))

        start_index = results['start_index']
        frame_inds = np.array(frame_inds) + start_index

        results['frame_inds'] = np.sort(frame_inds.astype(int)) if not results['flip'] else np.sort(frame_inds.astype(int))[::-1]
...
class SampleAVAFrames(SampleFrames):
    ...
    def __call__(self, results):
        ...
        frame_inds = np.array(frame_inds, dtype=int) + start_index
...
class RawFrameDecode:
    ...
    def __init__(self, io_backend='disk',with_bbox=False, pad=False, crop=False, reverse=0, decoding_backend='cv2', **kwargs):
        self.io_backend = io_backend
        self.decoding_backend = decoding_backend
        self.kwargs = kwargs
        self.scale=256
    ...
    def __call__(self, results):
        ...
        for frame_idx in results['frame_inds']:
            ...
            if modality == 'RGB':
                ...
                if self.with_bbox:
                    ...
                    # else:
                    x, y, w, h = bbox.split('_')
                    # print(filepath,x,y,w,h)
                    x, y, w, h = int(x), int(y), int(w), int(h)
                    # img_bytes = self.file_client.get(filepath)
                    # raw_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                    ...
                    if x <= 0:
                        x = 1
                    cur_frame_real = raw_frame[y:y2, x:x2]
                    #cv2.imshow("image", cur_frame_real)

                    frame_shape = cur_frame_real.shape
                    edge_ratio = (min(frame_shape[0], frame_shape[1])) / max((frame_shape[0], frame_shape[1]))
                    long_edge = self.scale
                    short_edge = int(self.scale * edge_ratio)
                    ...
                    cur_frame=mmcv.imresize(cur_frame_real,size=size)
                    cur_frame=mmcv.imresize(cur_frame,size=(256,256))
                    # filename=str(directory).rstrip('.mp4')
                    # filename=filename.lstrip('/home/zhy/data/')
                    #cur_frame_resize = mmcv.imresize(cur_frame_real, size=size, return_scale=False)
                    #cur_frame = mmcv.impad(cur_frame_resize, shape=(self.scale, self.scale), pad_val=0)
                    # cv2.imwrite('/home/zhy/zhy/result/img/' + str(filename) + '+' + str(frame_idx)+'.jpg', cur_frame)
                else:
                    img_bytes = self.file_client.get(filepath)
                    # Get frame with channel order RGB directly.
                    cur_frame = mmcv.imfrombytes(img_bytes, channel_order='rgb')
                    if self.crop and osp.split(osp.split(filepath)[0])[1].lower().startswith('right'):
                        cur_frame = cur_frame[60:1080, 780:1800, :]#TODO right视角开启
                    if self.pad and osp.split(osp.split(filepath)[0])[1].lower().startswith('rear'):
                        cur_frame = mmcv.impad(mmcv.imresize(cur_frame, (960, 540)), padding=(0, 0, 0, 130),padding_mode='reflect')
                        cur_frame = cur_frame[0:, 290:960, :]#TODO right视角开启
                    # cur_frame = mmcv.impad(mmcv.imresize(cur_frame,(960,540)),padding=(0,0,210,210))
                imgs.append(cur_frame)
            elif modality == 'Flow':
        ...
        if np.random.random(1)<self.reverse:#翻转的概率
            imgs.reverse()
        results['imgs'] = imgs
        results['original_shape'] = imgs[0].shape[:2]
        results['img_shape'] = imgs[0].shape[:2]
