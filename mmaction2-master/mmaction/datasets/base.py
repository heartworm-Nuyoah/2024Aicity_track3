        ...
        for metric in metrics:
            ...
            if metric == 'top_k_accuracy':
                ...
                if isinstance(topk, int):
                    topk = (topk, )
                top_k_acc = top_k_accuracy(np.concatenate(results), gt_labels, topk)
                log_msg = []
            ...
            if metric == 'mean_class_accuracy':
                mean_acc, each_class_acc = mean_class_accuracy(np.concatenate(results), gt_labels)
                eval_results['mean_class_accuracy'] = mean_acc
                log_msg = []
                log_msg.append(f'\nmean_acc\t{mean_acc:.4f}')
                for i in range(len(each_class_acc)):
                    log_msg.append(each_class_acc[i])
                log_msg = ''.join(log_msg)
                print_log(log_msg, logger=logger)
                continue
