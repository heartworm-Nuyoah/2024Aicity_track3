...
def mean_class_accuracy(scores, labels):
    ...
    for x_i in cf_mat:
        print("".join(["{:5d}\t".format(int(j_i)) for j_i in x_i]))
    each_class_acc = []
    print('Precision for Each CLASS: ')
    for i in range(len(cf_mat)):
        # each_class.append([(f"class_{i:d} : {cf_mat[i,i]/cf_mat[:,i].sum():.03f}")])
        # print_log(f"class_{i:d} : {cf_mat[i,i]/cf_mat[:,i].sum():.03f}")
        each_class_acc.append(f"class_{i:d} : {cf_mat[i,i]/cf_mat[:,i].sum():.03f}")
        print((f"class_{i:d} : {cf_mat[i,i]/cf_mat[:,i].sum():.03f}"))
    ...
    mean_class_acc = np.mean(
        [hit / cnt if cnt else 0.0 for cnt, hit in zip(cls_cnt, cls_hit)])
    for cnt, hit in zip(cls_cnt, cls_hit):
        if cnt:
            print(hit / cnt)
        else:
            print(0.0)
    return mean_class_acc,each_class_acc
